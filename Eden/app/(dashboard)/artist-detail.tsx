          ...
          <>
            ...
            <Card className="mb-4">
              <CardContent className="items-center py-6">
                <Avatar alt={currentArtist.name} className="w-32 h-32 mb-4">
                  ...
                  <AvatarFallback>
                    <Text className="text-4xl">{getInitials(currentArtist.name)}</Text>
                  </AvatarFallback>
                </Avatar>
                
                <Text className="text-3xl font-bold mb-2">{currentArtist.name}</Text>
                ...
                <View style={{backgroundColor:"transparent"}} className="flex-row items-center gap-2 mb-4">
                  <Mail size={16} className="opacity-70" />
                  <Text className="text-sm opacity-70">{currentArtist.email}</Text>
                </View>

                <View style={{backgroundColor:"transparent"}} className="flex-row gap-2 w-full">
                ...
                {currentArtist.profile && (
                  <>
                    <View style={{backgroundColor:"transparent"}} className="flex-row justify-between">
                      ...
                      <Text className="text-sm font-semibold">{currentArtist.profile}</Text>
                    </View>
                    <Separator />
                  </>
                )}
                ...
                {isLoadingStats ? (
                  ...
                  <>
                    <View style={{backgroundColor:"transparent"}} className="flex-row justify-between">
                      ...
                      <Text className="text-lg font-bold">{currentArtistStats.totalTracks}</Text>
                    </View>
                    <Separator />
                    
                    <View style={{backgroundColor:"transparent"}} className="flex-row justify-between">
                      <Text className="text-sm opacity-70">Published Tracks</Text>
                      <Text className="text-lg font-bold">{currentArtistStats.publishedTracks}</Text>
                    </View>
                    <Separator />
                    
                    <View style={{backgroundColor:"transparent"}} className="flex-row justify-between">
                      <Text className="text-sm opacity-70">Total Albums</Text>
                      <Text className="text-lg font-bold">{currentArtistStats.totalAlbums}</Text>
                    </View>
                    <Separator />
                    
                    <View style={{backgroundColor:"transparent"}} className="flex-row justify-between">
                      <Text className="text-sm opacity-70">Total Uploads</Text>
                      <Text className="text-lg font-bold">{currentArtistStats.totalUploads}</Text>
                    </View>
                    <Separator />
                    
                    <View style={{backgroundColor:"transparent"}} className="flex-row justify-between">
                      <Text className="text-sm opacity-70">Pending Uploads</Text>
                      <Text className="text-lg font-bold">{currentArtistStats.pendingUploads}</Text>
                    </View>
                  </>
                ) : (
            ...
            <Card className="mb-8">
              <CardHeader>
                ...
                {tracksPagination && (
                  <CardDescription>
                    Showing {currentArtistTracks.length} of {tracksPagination.total} tracks
                  </CardDescription>
                )}
              </CardHeader>
              <CardContent className="gap-2">
                {isLoadingTracks && currentArtistTracks.length === 0 ? (
                  ...
                  <>
                      ...
                      >
                        <Card className="mb-2">
                          <CardContent className="flex-row items-center py-3">
                            <View style={{backgroundColor:"transparent"}} className="w-12 h-12 rounded-md bg-primary/10 items-center justify-center mr-3">
                              {track.coverUrl ? (
                                <Avatar alt={track.title} className="w-12 h-12 rounded-md">
                                  <AvatarImage source={{ uri: track.coverUrl }} />
                                </Avatar>
                              ) : (
                            ...
                            <View style={{backgroundColor:"transparent"}} className="flex-1">
                              <Text className="font-semibold mb-1">{track.title}</Text>
                            ...
                            >
                              <Text className="text-xs capitalize">{track.status}</Text>
                            </Badge>
                          </CardContent>
                        </Card>
                      </Pressable>
                    ...
                    {tracksPagination && currentArtistTracks.length < tracksPagination.total && (
                      <Button 
                        variant="outline" 
                        className="mt-4"
                        onPress={() => {
                      ...
                      >
                        <Text>{isLoadingTracks ? 'Loading...' : 'Load More Tracks'}</Text>
                      </Button>
                    )}
                  </>
                ) : (
