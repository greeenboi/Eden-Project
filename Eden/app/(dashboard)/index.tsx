...
import { AlertCircle, Clock, Disc, Music, Play, Search, Settings, User, Users } from 'lucide-react-native';
...
export default function AllSongsScreen() {
  ...
  const { tracks, pagination, isLoading, error, fetchTracks, clearTracks } = useTrackStore();
  ...
  const filteredTracks = masonryTracks.filter(track =>
    searchQuery.trim() === '' ||
  ...
  const handleLoadMore = () => {
    if (pagination && !isLoading && pagination.page * pagination.limit < pagination.total) {
  ...
  const renderTrackCard = ({ item, index }: { item: MasonryTrack; index: number }) => {
    return (
      <Pressable 
      ...
      >
        <Card className="overflow-hidden">
          {/* Album Art / Cover */}
          <View style={{backgroundColor:"transparent"}} className={`w-full aspect-square ${getImageColors(index)} items-center justify-center`}>
          ...
          <CardContent className="p-3">
            ...
            <View style={{backgroundColor:"transparent"}} className="flex-row items-center gap-1 mb-2">
            ...
            <View style={{backgroundColor:"transparent"}} className="flex-row items-center justify-between">
              ...
              <View style={{backgroundColor:"transparent"}} className="w-8 h-8 rounded-full bg-primary items-center justify-center">
    ...
    <View style={styles.container}>
      ...
      {isLoading && tracks.length === 0 ? (
        <View style={{backgroundColor:"transparent"}} className="flex-1 p-4">
          <View style={{backgroundColor:"transparent"}} className="flex-row gap-2 mb-2">
        ...
        <View style={styles.listContainer}>
          <FlashList
            ...
            ListEmptyComponent={
              <View style={{backgroundColor:"transparent"}} className="py-8 items-center">
            ...
            ListFooterComponent={
              isLoading && tracks.length > 0 ? (
                <View style={{backgroundColor:"transparent"}} className="py-4 items-center">
