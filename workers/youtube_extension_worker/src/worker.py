...
async def root():
    return {
        "message": "YouTube Extension Worker - Use /metadata/spotify to get metadata"
    }
...
async def spotify_metadata(req: Request, payload: dict):
    ...
    results = await search_spotify_api(payload.get("query"), env, limit=5)

    if not results:
        raise HTTPException(status_code=404, detail="Track not found on Spotify")

    enriched = []
    for track_data, artist_data in results:
        artist_name = (
