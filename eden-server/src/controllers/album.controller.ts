...
export async function createAlbumHandler(c: ValidatedContext) {
	try {
		const body = c.req.valid("json") as z.infer<
			typeof CreateAlbumRequestSchema
		>;
...
export async function listAlbumsHandler(c: ValidatedContext) {
	try {
		const { page, limit, artistId } = c.req.valid("query") as {
			page: number;
			limit: number;
			artistId?: string;
		};
...
export async function updateAlbumHandler(c: ValidatedContext) {
	try {
		const { id } = c.req.valid("param") as { id: string };
		const body = c.req.valid("json") as z.infer<
			typeof UpdateAlbumRequestSchema
		>;
		const db = getDb(c.env);
...
export const deleteAlbumRoute = createRoute({
	...
	description:
		"Delete an album. Tracks will remain but lose album association.",
	request: {
		params: z.object({
			id: z.uuid().openapi({
				param: { name: "id", in: "path" },
