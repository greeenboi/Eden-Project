...
export async function createTrackHandler(c: ValidatedContext) {
	try {
		const body = c.req.valid("json") as z.infer<
			typeof CreateTrackRequestSchema
		>;
		...
		const track = {
			...
			album: trackWithRelations.album
				? {
						...
						releaseDate: serializeDateOrNull(
							trackWithRelations.album.releaseDate,
						),
...
export async function updateTrackHandler(c: ValidatedContext) {
	try {
		const { id } = c.req.valid("param") as { id: string };
		const body = c.req.valid("json") as z.infer<
			typeof UpdateTrackRequestSchema
		>;
		const db = getDb(c.env);
...
export const updateTrackStatusRoute = createRoute({
	...
	description:
		"Update track status (initiated → uploaded → processing → published)",
	request: {
		params: z.object({
			id: z.uuid().openapi({
				param: { name: "id", in: "path" },
		...
		body: {
			content: {
				"application/json": {
					schema: z.object({
						status: z
							.enum(["initiated", "uploaded", "processing", "published"])
							.openapi({
						...
						encodings: z
							...
							.openapi({
								description:
									"Encoding information (for processing/published status)",
...
export async function searchTracksHandler(c: ValidatedContext) {
	try {
		const { q: query, limit } = c.req.valid("query") as {
			q: string;
			limit: number;
		};
		const db = getDb(c.env);
			...
			content: {
				"application/json": {
					schema: z.object({
						...
						expiresAt: z.iso
							.datetime()
							.openapi({ description: "URL expiration timestamp" }),
						expiresIn: z
							.number()
							.openapi({ description: "Seconds until expiration" }),
