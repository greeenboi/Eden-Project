...
async function loadArtistsForUpload() {
  try {
    ...
    const select = document.getElementById('selectedArtist') as unknown as HTMLSelectElement
    if (!select) return

    if (artists.length === 0) {
      select.innerHTML = '<option value="">No artists available - create one first</option>'
      return
    }
...
function initFileUpload() {
  ...
  fileInput.addEventListener('change', async (e: Event) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      showAlert('Unsupported file type. Please upload MP3, WAV, FLAC, AAC, or OGG', 'error')
      return
    }
...
function initTrackMetadataForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/uploads/${currentUpload.uploadId}/complete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      showAlert(`Track "${metadata.title}" created successfully! Track ID: ${result.trackId}`, 'success')
