...
export async function requireAuth(
	...
	if (!token) {
		...
		return c.json(
			{ error: "AuthenticationError", message: "Missing authorization token" },
			401,
		);
	}
	...
	try {
		...
		if (error instanceof AuthenticationError) {
			...
			return c.json(
				{ error: "AuthenticationError", message: error.message },
				401,
			);
		}
		throw error;
...
export function requireRole(...roles: Array<"user" | "artist" | "admin">) {
	return async (
		...
		if (!user) {
			...
			return c.json(
				{ error: "AuthenticationError", message: "Not authenticated" },
				401,
			);
		}
